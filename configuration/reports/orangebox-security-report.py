...
def load_events(start,end,group):
    members=group_members(group); events=[]
    for path in iter_log_files(start,end):
        for outer in iter_json(path):
            event=parse_event(outer)
            if not event or not (start <= event["timestamp"] < end): continue
            if members is not None and event["agent_id"] not in members: continue
...
def firewall_data(events):
    adds={}; deletes={};
    for e in events:
        if e["outer_rule"] != FIREWALL_RULE or not e["srcip"]: continue
        key=(e["agent_id"],e["rule_id"],e["srcip"])
        if e["command"]=="add": adds[key]=e
        elif e["command"]=="delete": deletes[key]=e
    rows=defaultdict(lambda:{"ips":set()})
    for key,e in adds.items(): rows[(e["agent_id"],e["agent_name"],e["rule_id"],e["description"])] ["ips"].add(e["srcip"])
    result=[]
    security=[e for e in events if e["category"]!="other"]
    for key,row in rows.items():
        agent_id,agent_name,rule_id,description=key; ips=row["ips"]
        attempts=sum(1 for e in security if e["agent_id"]==agent_id and e["rule_id"]==rule_id and e["srcip"] in ips)
    ...
    return list(adds.values()),list(deletes.values()),sorted(result,key=lambda r:r["agent_name"].lower())
...
def generate_html(events,title,subtitle,period,group,lang="es"):
    L=labels(lang); security=[e for e in events if e["category"]!="other"]; critical=[e for e in security if e["level"]>=13]; categories={c:[e for e in security if e["category"]==c] for c in ("authentication","web","fim","malware","privilege","attack")}; firewall_adds,firewall_deletes,firewall_rows=firewall_data(events); all_ips={e["srcip"] for e in security if e["srcip"]}; agents=Counter((e["agent_id"],e["agent_name"]) for e in security)
    ...
    page.append("<table role='presentation' width='100%' cellpadding='0' cellspacing='0' border='0' style='width:100%;'><tr><td align='center' style='padding:12px;'><table role='presentation' width='640' cellpadding='0' cellspacing='0' border='0' style='width:100%;max-width:640px;background:#ffffff;border:1px solid #d5dde2;'>")
    ...
    def table_header(headers):
        return "<tr>"+"".join(f"<th align='left' style='background:#18313b;color:#fff;padding:8px;font-size:10px;text-transform:uppercase;'>"+esc(h)+"</th>" for h in headers)+"</tr>"
    def finish(section): return section+"</table></td></tr>"
    page.append(section_open("🛡️",L["firewall"],L["firewall_sub"]))
    if firewall_rows:
        page.append(table_header([L["agent"],L["reason"],L["rule"],L["attempts"],L["blocked_ips"]]))
        for row in firewall_rows:
            page.append("<tr>"+f"<td style='padding:8px;border-top:1px solid {border};font-size:11px;'>{esc(row['agent_name'])}</td>"+f"<td style='padding:8px;border-top:1px solid {border};font-size:11px;'>{esc(row['description'])}</td>"+f"<td style='padding:8px;border-top:1px solid {border};font-size:11px;'>{esc(row['rule_id'])}</td>"+f"<td style='padding:8px;border-top:1px solid {border};font-size:11px;'>{row['attempts']:,}</td>"+f"<td style='padding:8px;border-top:1px solid {border};font-size:11px;'>{len(row['ips']):,}</td></tr>")
        total_ips=len({ip for row in firewall_rows for ip in row["ips"]}); total_attempts=sum(row["attempts"] for row in firewall_rows)
        page.append(f"<tr><td colspan='5' style='padding:10px 8px;background:#f8fafb;border-top:1px solid {border};font-size:11px;color:{muted};'><b>{total_ips:,}</b> IPs bloqueadas automáticamente · {total_attempts:,} detecciones asociadas.</td></tr>")
    else: page.append(f"<tr><td style='padding:12px;font-size:11px;color:{muted};'>{esc(L['no_firewall'])}</td></tr>")
    page.append(f"<tr><td style='padding:10px 12px;color:{muted};font-size:10px;border-top:1px solid {border};'>{esc(L['firewall_note'])}</td></tr>")
    page.append(finish(""))
    for category,icon in (("authentication","🔐"),("web","🌐"),("fim","📁"),("malware","🦠"),("privilege","⬆️"),("attack","⚠️")):
        subset=categories[category]; page.append(section_open(icon,L[category],L.get(category+"_sub")))
        if subset:
            page.append(table_header([L["rule"],"Descripción",L["detections"],L["agents"]]))
            for rid,desc,count,names in section_rows(subset):
                page.append("<tr>"+f"<td style='padding:8px;border-top:1px solid {border};font-size:11px;'>{esc(rid)}</td>"+f"<td style='padding:8px;border-top:1px solid {border};font-size:11px;'>{esc(desc)}</td>"+f"<td style='padding:8px;border-top:1px solid {border};font-size:11px;'>{count:,}</td>"+f"<td style='padding:8px;border-top:1px solid {border};font-size:10px;'>{esc(', '.join(names[:5]))}</td></tr>")
        else: page.append(f"<tr><td colspan='4' style='padding:12px;font-size:11px;color:{muted};'>{esc(L['no_activity'])}</td></tr>")
        if category=="attack": page.append(f"<tr><td colspan='4' style='padding:10px 12px;color:{muted};font-size:10px;border-top:1px solid {border};'>{esc(L['attack_note'])}</td></tr>")
        page.append(finish(""))
    page.append(section_open("🎯",L["mitre"]))
    mitre=mitre_rows(security)
    if mitre:
        page.append(table_header([L["technique"],L["meaning"],L["detections"]]))
        for mid,name,meaning,count in mitre:
            page.append("<tr>"+f"<td style='padding:8px;border-top:1px solid {border};font-size:11px;white-space:nowrap;'>{esc(mid)}<br><b>{esc(name)}</b></td>"+f"<td style='padding:8px;border-top:1px solid {border};font-size:10px;'>{esc(meaning)}</td>"+f"<td style='padding:8px;border-top:1px solid {border};font-size:11px;'>{count:,}</td></tr>")
    else: page.append(f"<tr><td style='padding:12px;font-size:11px;color:{muted};'>{esc(L['no_activity'])}</td></tr>")
    page.append(f"<tr><td colspan='3' style='padding:10px 12px;color:{muted};font-size:10px;border-top:1px solid {border};'>{esc(L['mitre_note'])}</td></tr>")
    page.append(finish(""))
    page.append(f"<tr><td style='padding:14px 20px;background:{dark};color:#c6d2d7;font-size:10px;line-height:1.5;'>OrangeBox IT Services · Wazuh Security Monitoring<br>{esc(L['technical_note'])}</td></tr></table></td></tr></table></body></html>")
    return "".join(page)

def archive_report(content,period,group):
    Path(ARCHIVE_DIR).mkdir(parents=True,exist_ok=True); stamp=datetime.now().strftime("%Y%m%d-%H%M%S"); safe=re.sub(r"[^A-Za-z0-9_.-]+","_",group); path=Path(ARCHIVE_DIR)/f"security-report-{period.lower().replace(' ','-')}-{safe}-{stamp}.html"; path.write_text(content,encoding="utf-8"); return path

def send_email(content,subject,recipient):
    msg=MIMEMultipart("alternative"); msg["Subject"]=subject; msg["From"]=DEFAULT_FROM; msg["To"]=recipient; msg.attach(MIMEText(content,"html","utf-8"))
    with smtplib.SMTP(SMTP_HOST,SMTP_PORT,timeout=30) as smtp: smtp.sendmail(DEFAULT_FROM,[recipient],msg.as_string())

def main():
    parser=argparse.ArgumentParser(description="Informe OrangeBox de actividad de seguridad Wazuh")
    period=parser.add_mutually_exclusive_group(required=True)
    for option in ("today","yesterday","thisweek","lastweek","thismonth","lastmonth","thisyear","lastyear"): period.add_argument(f"--{option}",action="store_true")
    parser.add_argument("--date",metavar="YYYY-MM-DD"); parser.add_argument("--group",default="all"); parser.add_argument("--email",action="append",required=True,help="Destinatario; puede repetirse o contener una lista separada por comas"); parser.add_argument("--lang",choices=("es","en"),default="es")
    args=parser.parse_args()
    if args.date: mode=f"date:{args.date}"
    else: mode=next(name for name in ("today","yesterday","thisweek","lastweek","thismonth","lastmonth","thisyear","lastyear") if getattr(args,name))
    now=datetime.now().astimezone(); start,end,period_label=period_bounds(mode,now); events=load_events(start,end,args.group); title=labels(args.lang)["report"]; subtitle=labels(args.lang)["subtitle"]; content=generate_html(events,title,subtitle,period_label,args.group,args.lang); archive=archive_report(content,period_label,args.group)
    recipients=[]
    for value in args.email:
        recipients.extend(part.strip() for part in value.split(",") if part.strip())
    recipients=list(dict.fromkeys(recipients)); subject=f"OrangeBox Wazuh · {title} · {period_label}"
    failures=[]
    for recipient in recipients:
        try: send_email(content,subject,recipient); print(f"Correo enviado a {recipient}")
        except Exception as exc: failures.append((recipient,exc)); print(f"ERROR enviando a {recipient}: {exc}")
    print(f"Reporte archivado: {archive}")
