...
LABELS_ES = {
    ...
    "agents":"Sistemas más afectados", "details":"Ver IPs", "no_firewall":"No se registraron bloqueos automáticos con una IP de origen válida.", "no_activity":"No se registraron detecciones de esta categoría durante el período.",
...
LABELS_EN = {
    ...
    "access":"Access attempts", "web":"Web access and reconnaissance attempts", "fim":"Detected file changes", "malware":"Malware and suspicious file detections", "priv":"Privilege escalation", "attack":"Detected attack attempts", "mitre":"Techniques associated with attack attempts", "technique":"Technique", "meaning":"What it means", "detections":"Detections", "agents":"Most affected systems", "details":"View IPs", "no_firewall":"No automatic blocks with a valid source IP were recorded.", "no_activity":"No detections were recorded for this category during the period.", "technical_note":"Rule identifiers and descriptions come from the Wazuh detection engine. Detecting an attempt does not by itself mean that the system was compromised.",
}
...
def iter_log_files(start,end):
    ...
    while d<=last:
        base=Path(ALERTS_ROOT)/f"{d.year:04d}"/d.strftime("%b"); paths += [base/f"ossec-alerts-{d.day:02d}.json.gz",base/f"ossec-alerts-{d.day:02d}.json"]; d+=timedelta(days=1)
...
def firewall_data(events):
    ...
    rows=defaultdict(lambda:{"ips":set(),"times":[]}); security=[e for e in events if e["outer_rule"]!=FIREWALL_RULE]
    for e in adds:
    ...
    for key,row in rows.items():
        aid,name,rid,desc=key; ips=row["ips"]
        attempts=sum(1 for e in security if e["agent_id"]==aid and e["rule_id"]==rid and e["srcip"] in ips)
        if attempts==0: attempts=len(ips)
        result.append({"agent_id":aid,"agent_name":name,"rule_id":rid,"description":desc,"ips":sorted(ips,key=lambda x:(ipaddress.ip_address(x).version,ipaddress.ip_address(x))),"attempts":attempts,"first":min(row["times"]) if row["times"] else None,"last":max(row["times"]) if row["times"] else None})
...
def generate_html(events,title,subtitle,period,group,lang="es"):
    ...
    if fw_rows:
        ...
        for row in fw_rows:
            iphtml=''.join(f"<span style='display:inline-block;background:#fff;border:1px solid #d9e2e6;padding:2px 5px;margin:2px;font-family:monospace;font-size:10px;'>{esc(ip)}</span>" for ip in row['ips'])
            detail=f"<details style='margin-top:5px;'><summary style='cursor:pointer;color:#425d68;font-size:10px;font-weight:bold;'>{esc(L['details'])} ({len(row['ips'])})</summary><div style='margin-top:6px;background:#f7f9fa;border:1px solid #dce5e9;padding:7px;overflow-wrap:anywhere;'>{iphtml}</div></details>"
